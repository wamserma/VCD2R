...
It contains 1024 executions of a simplified last [AES](http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf) round. Data is combined with a key byte, then the S-Box is applied, then another key addition is done. The S-Box-Output is stored in registers.
...
We can view the activity using either [plotly](https://plot.ly/javascript/) or [dygraphs](http://dygraphs.com/). Using the latter we get:
...
Here we mark the start/end of each S-Box-invocation. To make things easier, a trigger signal can be included (in the testbench).Usually the attacked device will be reset before it is fed a new value. So the Reset-Signal can be used as a trigger. Re-alignment of traces is not an issue with simulated traces. The reset from the testbench is used as a trigger in the subsequent examples.
...
graph$plot
```
(Drag the sliders in the bottom part or mark a range of the graph to zoom in.)
...
To follow this attack, two things need to be noted: First, the long trace is not explicitly split into individual traces. Rather the trigger signal is used as an index. Second, it is not the combinatorial part, that is attacked, but rather the output of the combinatorics that is written so a sequential cell at the end.
...
The stimuli (inputs for the circuit) are loaded from a file.
...
```{r initial.cor.over.time,fig.width=6}
...
```{r ref.label="initial.cor.over.time",fig.width=6}
...
```{r init.cors.hw.plot, fig.width=6 }
plot(1:1000,cors[,1],type="l",col="gray",xlab="# of traces", ylab= "correlation",ylim=c(0,max(cors,na.rm = T)))  
for (i in 2:256) lines(1:1000,cors[,i],col="gray") # all keys
lines(1:1000,cors[,214],col="blue") # guessed key, "pearson"
```
...
```{r, ref.label="init.cors.hw.plot", warning=FALSE, fig.width=6}
cors2<-outer(1:1000,c("kendall","spearman"),Vectorize(function(samples,meth) {
...
# just for reference, the keys used for generating the traces
...
```{r echo=F, fig.width=6,warning=FALSE}
...
To demonstrate the power of MI we take the leakage during computation of the S-Box. The correct key is also recovered in this case, for a slightly modified leakage function: The affine part (xor of 0x63) of the SBox is xor'd to the key.
...
With the mighty statistical toolset of R at our hands, we can try many more attacks. We applied Correlation and Mutual Information Analysis, we could try Regression Models, Clustering, Random Forests (e.g. for traces that leak at multiple/many time instances), or even use combinations of those!
